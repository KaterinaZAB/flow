...
for(const name of readdirSync(resolve('drizzle')).filter(n=>n.endsWith('.sql')).sort()){
 ...
 sqlite.exec('BEGIN');try{sqlite.exec(readFileSync(resolve('drizzle',name),'utf8'));sqlite.prepare('INSERT INTO local_migrations VALUES (?)').run(name);sqlite.exec('COMMIT')}catch(e){sqlite.exec('ROLLBACK');throw e}
 }
}
class Stmt {
...
export const env={DB:{prepare:(sql:string)=>new Stmt(sql),async batch(stmts:Stmt[]){sqlite.exec('BEGIN');try{const result=[];for(const st of stmts){result.push(await st.run())}sqlite.exec('COMMIT');return result}catch(e){sqlite.exec('ROLLBACK');throw e}}}};
