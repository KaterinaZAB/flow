...
export function CandidateList({ initial, transactions, guest = false, }: {
    initial: Candidate[];
    transactions: Transaction[];
    guest?: boolean;
}) {
    const [items, setItems] = useState(initial), [selected, setSelected] = useState<Set<string>>(() => new Set(initial
        .filter((c) => (c.expense.confidence ?? 0) >= 0.8)
        .map((c) => c.id))), [busy, setBusy] = useState(false), [error, setError] = useState(''), [finished, setFinished] = useState(false);
    async function action(ids: string[], decision: string, edit?: Record<string, unknown>) {
        setBusy(true);
        setError('');
        try {
            const r = await localAction('/api/candidates', {
                method: 'POST',
            ...
            const updated = await localAction('/api/candidates', { cache: 'no-store' });
            if (!updated.ok)
                throw new Error('Не удалось обновить результат.');
            const data = (await updated.json()) as Candidate[];
            setItems(data);
            setSelected((prev) => new Set([...prev].filter((id) => data.some((c) => c.id === id))));
            if (!data.length)
                setFinished(true);
        }
        catch (e) {
    ...
    const act = (ids: string[], decision: string) => void action(ids, decision).catch(() => { });
    return (<>
      <div className="page-heading">
        <div>
          ...
          <h1>
            {items.length
            ? 'Мы нашли ' + items.length + ' возможных расходов'
            : finished
                ? 'Ваш обзор готов'
          ...
          <p>
            {items.length
            ? 'Проверьте результат. Мы могли ошибиться.'
      ...
      {!items.length ? (<div className="panel empty-state">
          ...
          <div className="selection-bar">
            <label>
              <Checkbox checked={selected.size === items.length} onCheckedChange={(checked) => setSelected(new Set(checked ? items.map((c) => c.id) : []))} aria-label="Выбрать все расходы"/>
              Выбрать все
            </label>
            <span>
          ...
          <div className="candidate-grid">
            {items.map((c) => {
                ...
                return (<section className={'panel candidate ' + (selected.has(c.id) ? 'selected' : '')} key={c.id}>
                  <div className="candidate-top">
                    <Checkbox aria-label={'Выбрать ' + e.name} checked={selected.has(c.id)} onCheckedChange={(value) => setSelected((prev) => {
                    ...
                    <ServiceLogo name={e.name} serviceId={e.serviceId} type={e.type}/>
                    <div>
                      <h3>{e.name}</h3>
                      <p>
                  ...
                  <div className="candidate-actions">
                    {(<button disabled={busy} className="primary-button" onClick={() => act([c.id], 'confirmed')}>
                        <Check size={15}/>
                        Подтвердить
                      </button>)}
                    <ExpenseForm expense={e} buttonLabel="Изменить" onSave={(data) => action([c.id], 'edit', data)} guest={guest}/>
                    {(<button disabled={busy} className="reject-button" onClick={() => act([c.id], 'rejected')}>
                        <X size={15}/>
                        Не регулярный
                      </button>)}
                  </div>
          ...
          <div className="confirmation-bar">
            ...
            {(<button disabled={busy || !selected.size} className="primary-button" onClick={() => act([...selected], 'confirmed')}>
                {busy
                    ? 'Сохраняем…'
