...
export function divideRounded(n: number, d: number): number {
  if (!Number.isSafeInteger(n) || !Number.isInteger(d) || d <= 0)
    throw new Error('Invalid money operation');
  return Number((BigInt(n) * 2n + BigInt(d)) / (2n * BigInt(d)));
}
export function money(n: number, currency = 'RUB', decimals = false) {
  return new Intl.NumberFormat('ru-RU', {
    style: 'currency',
    currency,
    maximumFractionDigits: decimals || n % 100 !== 0 ? 2 : 0,
